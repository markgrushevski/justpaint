...
free-draw editor (`/draw`) is a supporting mode. Greenfield; the game is Phase 3 (not built yet),
the Go backend + minimal editor (Phase 1) are done. A **Go + TS monorepo** (npm workspaces for the
TS side; the Go service is separate).
