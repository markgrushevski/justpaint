...
function backgroundLayer(color: string, width: number, height: number): Konva.Layer {
    ...
    layer.add(new Konva.Rect({ x: 0, y: 0, width, height, fill: color }))
    return layer
}
 ...
 */
export function stageConfig(container: HTMLDivElement | undefined, width: number, height: number): Konva.StageConfig {
    const el = container ?? (typeof document !== 'undefined' ? document.createElement('div') : undefined)
