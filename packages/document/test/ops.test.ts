...
describe('validateOpBatch', () => {
    ...
    const overBatchCap = Array.from({ length: LIMITS.maxOpsPerBatch + 1 }, (_v, i) => ({
        kind: 'add_layer',
        id: `L${i}`,
        name: 'n'
    }))
    ...
    const invalid: Array<[string, DocSummary, unknown]> = [
        ...
        [
            'add_stroke polygon under three vertices',
            summaryWith('L1'),
            opsWith('L1', {
                id: 's1',
                type: 'polygon',
        ...
        [
            'add_layer name too long',
            summaryWith(),
            [{ kind: 'add_layer', id: 'x', name: 'x'.repeat(LIMITS.maxNameLen + 1) }]
        ],
