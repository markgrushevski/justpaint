import type { Composite, DocSummary, Document, LineCap, LineJoin, Op, StrokeType } from './types'
...
const STROKE_TYPES: readonly StrokeType[] = ['freehand', 'line', 'rect', 'ellipse', 'polygon']
 ...
 */
function checkPointArray(value: unknown, path: string, arity: number, min: number): number {
...
function validateLayer(value: unknown, path: string, ids: Set<string>): unknown[] {
    ...
    checkId(value.id, `${path}.id`, ids)
    if (typeof value.name !== 'string' || runeLen(value.name) < 1 || runeLen(value.name) > LIMITS.maxNameLen) {
...
export type ValidationResult = { ok: true; document: Document } | { ok: false; error: DocumentValidationError }
    ...
    ops.forEach((op, i) => {
        ...
        switch (op.kind) {
            case 'add_layer': {
                ...
                if (typeof op.name !== 'string' || runeLen(op.name) < 1 || runeLen(op.name) > LIMITS.maxNameLen) {
                    fail(`${path}.name`, `must be 1-${LIMITS.maxNameLen} chars`)
                }
...
export type OpValidationResult = { ok: true; ops: Op[] } | { ok: false; error: DocumentValidationError }

/** Non-throwing wrapper around {@link validateOpBatch}. */
export function safeValidateOpBatch(summary: DocSummary, ops: unknown): OpValidationResult {
    try {
