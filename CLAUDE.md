...
- **AI Assist Phase A is shipped** (`feat/assist-phase-a`, 2026-07-13) — the first AI-in-product slice: a prompt → validated `Op` batch (`add_layer`/`add_stroke`) → ghost preview → one composite editor command, via a judge-style `internal/assist` seam (`FakeAssist` live; `AnthropicAssist` a config-gated scaffold only, no live SDK call). See `docs/ASSIST.md`. **Phase 4** (stretch: ratings + leaderboard shipped; realtime hardening, teams/tournaments, replay, and the real judge integration remain) is in progress — see `docs/ROADMAP.md`.
...
- **CI:** `.github/workflows/ci.yml` runs these same gates on push to `main` and on every PR — a **ts** job (format:check / types / test / build / `lint:ci` / render selftest) and a **go** job (gofmt / vet / build / goose + `go test` against a postgres service). Green locally = green in CI.
