...
>
> The async duel is **v1**; live realtime (§9) is a delivery upgrade over it, not a fork of the lifecycle, and single-player practice (§10) is a separate mode alongside it. Companions: `docs/DECISIONS.md` (the "why"), `docs/ARCHITECTURE.md` (§7 data model, §8 async-first), `docs/JUDGE.md` (scoring contract), `docs/API.md` (routes, error shape, DoS caps). When this disagrees with those for what it owns, this doc wins; for what it defers, they win.
...
The `drawing` round carries a **server-authoritative deadline**: `matches.drawing_deadline` is stamped `now() + 90s` on Postgres' own clock the instant `open → drawing` fires, enforced both by a background sweeper and defensively on every `submit` — so a round resolves even when nobody is polling.
...
| `drawing` → `judging` | The **last** outstanding player **submits** (before the deadline). | Persist each submission as a `drawings` row (`match_id` set), stamp `match_players.drawing_id` + `submitted_at`. When all roster slots have a submission, flip `status = 'judging'` and kick off rendering. The flip is also where the AI-call ledger bills the provider for the request it is about to cause (§4.3) — every path into `judging` goes through the same wrapper. |
...
| `open` → `abandoned` | An **open match sits unjoined past a TTL** (~10 min). | A background reaper sweeps stale `open` matches to `abandoned` so a ghost match can't later pair a fresh joiner against a creator who's long gone. |

Notes:
- **`aborted` is a last resort, not a retry policy.** The stuck-judging watchdog re-fires a stale attempt up to the cap first; only a match that burned every retry is closed out this way, and it is logged at error level as a judge-infrastructure failure worth an alert. The two sweeps are exact complements — below the cap the row is re-fired, at or past it the row is aborted — so no wedged match can fall between them (`DECISIONS.md` 2026-09-18).
...
- **The judge is billed at the flip into `judging`, not at the start of the round** (§4.3). So the two deadline resolutions above cost the provider nothing — a forfeit and an abandonment never reach a judge — and a stuck-judging re-fire, which enters `judging` again, is billed again. The players are billed once either way, when their round was granted.
...
Every `POST /api/matches` call, **every `POST /api/practice` run** (§10), **every `POST /api/guess`** (`API.md` §13) **and every `POST /api/assist/ops`** (`ASSIST.md` §3.4) is gated on a **daily AI-call budget** before any expensive work happens — for a match, checked once before any of the three `CreateOrJoin` branches above run; for a practice run, checked once at the very top of `practice.Service.Run`, before the render or the critic is ever reached; for a guess, checked at the top of `guess.Service.Guess` too, second only to the "is a guesser even wired" test, so an unconfigured feature never consults a budget for a call it will never make; for an assist call, checked last of that handler's guards and immediately before the one line that costs money. All four ask the same `internal/aibudget` (`server/internal/aibudget/`), each holding it as plain funcs bound to its own **kind** at the composition root — never a second copy — so features that spend the same external quota are counted by **one rule**, never two that could drift. Practice, guess and assist hold a check/spend **pair** (`aibudget.For`); the duel holds that same check plus the **two** billing ports its split halves need (`aibudget.ForSplit` — `BillPlayers` and `BillProvider`, both taking the caller's own transaction), for the reason the next paragraph gives. For a match this sits ahead of the matchmaking transaction, since these are advisory reads and that transaction holds row locks worth keeping short. It exists because the per-IP rate limiter (`API.md` §3.1) bounds request *rate*, not the resource actually at risk: an AI provider runs on a quota measured in requests per **day**, a practice run / guess / assist call costs exactly one request and a duel one per judging pass, and the write tier alone (30/minute) lets a single IP threaten a whole day's quota within minutes (`DECISIONS.md` 2026-09-19, 2026-09-20). That check is not the only gate: on practice, guess and assist the ledger **write** re-tests the same per-kind allowance at the instant it writes, so a `429` can also arrive *after* the server-side render rather than only before it (see "How exact the two halves are" below).

**Counted over one ledger table, not derived from the domain tables** (`server/migrations/00007_ai_calls.sql`, `server/internal/db/queries/ai_calls.sql`). The budget used to be a union over whichever tables a feature happened to write — `matches` / `match_players` for a duel, `practice_runs` for a practice run — each arm carrying its own judgement about *which lifecycle column means a call was actually spent*. Two of the AI features cannot be counted that way at all: a `/draw` **guess** has no row anywhere, because the drawing may never be saved, and **assist** has no row either and was bounded only by an in-process token bucket that the host resets on every deploy and every wake from idle — so that ceiling had never actually held. Every spend now appends to `ai_calls`, in a uniform shape with no special cases — but the two shapes are two different **facts**, and they do not always happen at the same moment. A **provider row** (`user_id` null, `provider` set) says *one request to the provider is about to be made*; a **player row** (`user_id` set, `provider` null) says *this player was granted a round of this kind*. For practice, guess and assist the two are simultaneous, so one statement writes both: **2** rows per call, or none. For a duel they are not, and each half is written where it is true — **two player rows** when the roster fills, and **one provider row per judging pass**, minutes later, possibly more than once, possibly never. So an ordinary duel still writes **3** rows; a **forfeited or abandoned** one writes **2** (no judge ever runs, so no request is billed); one whose stuck-judging watchdog re-fires once writes **4**. That split is what lets one duel cost the provider a single request while costing two players a day's allowance each, with no `DISTINCT`, no divisor and no join; a check constraint (`ai_calls_bills_something`) rejects a row that bills neither.

Two independent halves, both must clear:

- **Per-user, and per KIND** (`aibudget.KindSpentError` → `429`, checked first): how many calls of *this kind* this player has spent inside the rolling window. Per-kind, not one shared pot across every AI feature: spending a day's duels no longer costs a player their assists, because the two are not substitutes for each other and one shared number could only ever be wrong for one of them. The honest cost: a single player's exposure across the Google-backed kinds is 20 + 20 + 2 + 40 = **82/day** with assist on the same provider (`ASSIST_MODE=gemini`, `ASSIST.md` §3.2), which is far above the global ceiling below. On a free key the per-player caps are therefore **not what binds** — the global half is, and it is meant to be: it is the one that stands between one enthusiastic afternoon and a feature that is off for everybody. For the single-actor kinds this half is enforced **twice**: by the advisory read above, and again by the ledger write itself, since `RecordAICallUnderCap` is a single conditional statement that inserts its two rows only while the caller is under the cap — it writes 2 rows or 0, and **0 is the refusal**, returning the same `*KindSpentError` the read returns. A **duel's** player rows stay unconditional, because there is nobody there to refuse: the joiner was already checked before the transaction opened, and the other seat is being granted a round by somebody else's action, so dropping their row would undercount a round that really happened and failing the statement would strand a match on a cap that is not the caller's.
- **Global, and per PROVIDER *and MODEL*** (`aibudget.ErrGlobalSpent` → `429`, checked second, only once the caller clears their own cap): how much of *one pool's* quota this service has spent inside the same window, across every kind drawing on it. Per provider, never service-wide — one vendor running dry must not refuse a feature backed by another that still has quota — and, where the vendor meters per model, **per model too**: the ledger key is `google:<model>` (`aibudget.Provider.WithModel`), so two kinds pinned to two models (`AI_MODEL_PER_KIND`) count two independent pools instead of adding two quotas together and refusing against a budget neither had spent. A provider that is *not* metered per model keeps its bare name: the judge service is one service however many models sit behind it, and splitting its counter would only hide how much of it we are using. That is not a tidiness argument — it is the measurement in **Defaults** below: Google's free tier is metered *per project per model*, so our number and theirs are only comparable when both count the same thing. This is also the one refusal nobody outside can see, so it is the one the budget logs (`Warn`): every feature backed by that provider is off for **everyone** until the window rolls. Unlike the per-user half it stays **advisory** — it is read at the check and never gates a write. By the time a call is billed the expensive, human part has already happened (a duel's round has been played), so refusing at the insert would strand work rather than prevent it; the ceiling is set *below* the provider's own quota precisely so a small overshoot stays inside it — a margin that is thinner than it sounds on a free key, for the reason **Defaults** gives.

Per-user is checked first because it is the case that actually fires and it is the cheaper read; a player over their own cap is told exactly that, never anything about the service's remaining budget. The **global** refusal discloses nothing about the budget's size — that is operator information — while a per-kind refusal **always names the player's own cap**, because that number is theirs: they could have counted it themselves, and *"you have used all 2 of your AI guesses for today"* rather than *"all of them"* is the difference between a limit and a shrug. Every kind now says it through **one template** with the kind's noun substituted in (`msgKindSpentFmt` + `Kind.Noun` — "duels", "scored drawings", "AI guesses", "AI drawing requests"), not one message constant per feature: a constant per kind was a chance per kind for one to drift, which is exactly how `guess` ended up the only kind naming its number (`API.md` §8/§10/§12/§13; the copy lives in one place, `server/internal/aibudget/http.go`).

**Spending is explicit, and it happens BEFORE the provider call** — never inferred afterwards from a lifecycle column. Each row is written **where its fact is true**, which for a duel means two different moments:

- A **duel's players** are billed inside the matchmaking transaction, at the one site that grants a round: `open → drawing`, the join branch. **Both** players are billed for that one round, from the roster that transaction just read — nobody is drawn into a duel passively, both sides pressed play, and both get the round. The branches that do *not* start a round bill nothing, so "a match nobody ever joined costs the player nothing" is true **by construction** rather than by a query's choice of anchor column.
- A **duel's provider row** is written later and separately, at every site that enters `judging` and at no other: the last submit, the deadline path's defensive both-submitted branch, and the stuck-judging re-fire. All three go through one wrapper around `SetMatchJudging` (`game.Service.enterJudging`), which is what makes "one provider row per judging pass" a property of the code rather than a hope. Starting a round is *not* a request to the judge — it becomes one only if both players submit — so a **forfeited or abandoned** round bills no provider row at all, while a re-fire, which really is a second request, bills one more. The row commits with the flip, so a pass that never became a pass is never billed, and a ledger failure fails the transition rather than letting the ceiling silently stop counting.
- A **practice run** and a **guess** are both billed **between the render and the model call**, and an **assist** call immediately before the impl is invoked — all outside any transaction, deliberately. Never *after* the call: a critique or a guess that failed still spent the provider's quota, and a failure the budget cannot see is exactly what a broken impl drains it through — a transaction would roll the record back for the same reason and be just as wrong. But not *before* the render either, and that is the narrower half of the window: the render is our own subprocess and spends nobody's quota, so a renderer that fell over must not cost a player a scored drawing, or one of the two guesses they get for the day. Everything above that line is free to re-run; nothing below it is. This write is also the **tighter of the two per-user gates** — it refuses on the count as it stands at the instant of writing, where the check read one a whole render earlier — and its refusal is the same `429` with the same sentence, so no caller needs a second branch for it.

Every ledger write runs under its own **5s timeout**, on the caller's context rather than inside the render/call work budget: a nearly-spent work budget must never be the reason a call that was made goes uncounted, and a single-row insert on the database the request already used is slow only when something is genuinely wrong.

**Window:** rolling 24h, not a calendar day — no timezone to get wrong, and a rolling window is the strictly tighter reading: never exceeding a ceiling in *any* 24h also never exceeds it in whatever calendar day the provider counts. What that argument does **not** give you is safety against the provider's own quota, and the Defaults below say why: the number that would have to sit under theirs is counted in ledger rows, and a row is a judging *pass*, not a request. It is measured by **Postgres**, against the database's own `now()`, so the window cannot skew when the app clock and the DB clock disagree.

**Defaults:** **15** global *per provider and model* (`AI_DAILY_GLOBAL`, `config.DefaultAIDailyGlobal`), and per player per rolling day **duel 20, practice 20, guess 2, assist 40** (`aibudget.DefaultPerUser`), overridden per kind by `AI_DAILY_PER_USER="duel=20,guess=2"`.

The global number **was 200 until 2026-09-20**, chosen against an unverified belief about the free tier. Then the API answered the question itself, in the body of a `429`:
...
**Twenty requests a day, per project, per model.** That measurement is what put the model into the ledger key (above) and what dropped the default by more than an order of magnitude — and it is worth being blunt about what 15 does and does not buy:

- **On a free key it is not the ceiling that binds.** Google's is, and it arrives as `judge.ErrQuotaExhausted`, which every synchronous handler now turns into the same calm refusal our own ceiling produces (`JUDGE.md` §8.1). Ours is the one that keeps **one player** from spending the day's quota in a burst; theirs is the one that actually ends the day.
- **15 is not "safe" either**, and not by a small margin. The ledger records one row per judging **pass**, while a pass may retry up to `geminiMaxAttempts` (**3**) times inside the seam (`JUDGE.md` §7) — so 15 rows are somewhere between **15 and 45 real requests**, which straddles a 20/day pool in both directions. Counting the retries would mean handing the frozen `Judge` contract a database dependency, which is not a trade worth making (see "How exact the two halves are").
- **Raise it the moment the key is not a free one.** 15 is sized to a specific free tier measured on one day; it is not a statement about what this service costs to run.

Duel and practice keep the old shared 20 because they are the same act with and without an opponent; guess is deliberately tiny — a novelty question about a drawing that may never be saved, otherwise the cheapest way to empty a provider quota — and assist is the loosest because it is a working tool, and a ceiling that interrupts a drawing session breaks the feature rather than bounding it. Every value is rejected below 1 at boot — there is no "unlimited" setting — and a per-user cap set *above* the global one is kept legal on purpose, as the idiom for "no real per-player limit, the global budget is the only ceiling." The pre-per-kind names still work, so a deployed environment needs no edit (`server/.env.example`): `JUDGE_DAILY_BUDGET` is a straight **alias** of `AI_DAILY_GLOBAL` (both set to *different* values is a boot error naming both), and `JUDGE_DAILY_PER_USER` seeds **only** `duel` and `practice` — the two kinds that shared it when it was named — so an operator's old `20` never quietly becomes 20 AI guesses a day.

**Enforced only for a kind whose impl really calls a provider.** A kind with no provider is never checked and never recorded — you cannot spend a quota you have no provider for. Which provider backs which kind is resolved once, at the composition root (`aibudget.Policies`, `server/cmd/server/main.go`), read from **the impl that was actually built** (`assist.CallsProvider`) rather than from the mode env alone, since an impl that calls nobody must never be billed as if it did.

Today: `JUDGE_MODE=gemini` puts duel, practice *and* guess on `google:<model>`, each kind on the model `AI_MODEL_PER_KIND` resolves for it (the same model id means the same pool; a kind given its own model gets its own); `JUDGE_MODE=http` puts the duel on `collaborator` and leaves practice *and* guess unprovidered, since that mode has neither a critic nor a guesser (`JUDGE.md` §8.2, §8.3); `ASSIST_MODE=gemini` puts assist on `google:<model>` too (`ASSIST.md` §3.4). Every `fake` impl has no provider, so dev and CI never meet a ceiling — which puts one player's Google-backed exposure at 82/day when every Gemini-backed kind is enabled.

Two boot checks keep that configuration honest, because an inert ceiling and an enforced one look identical from the outside: an allowance set (`AI_DAILY_PER_USER`) for a kind whose impl calls nobody is **inert** — not an error, since a deployment may carry a ceiling for a feature it has switched to a fake, but logged (`Warn`) so nobody believes a number nothing reads; and a kind that *does* have a provider whose allowance is below 1 is a boot **error**, because it would refuse every call of that kind, which is never what configuring a ceiling meant.

**How exact the two halves are.** The per-user half is enforced by the INSERT (above), so it no longer depends on a read taken a render and a network round trip earlier. The global half is advisory by design. Neither is exact, and the honest statement of what remains is:

- **A per-user cap can still be overshot by concurrent writes — by far less than it used to be.** The old shape was a check, then a render, then a provider call, with nothing re-testing the cap in between: measured, a per-kind cap of **2** billed **24 of 25** simultaneous requests — a **12×** overshoot, not "a handful". Moving the condition into the insert shrinks the window from seconds to the duration of one statement: re-measured, the same 25 simultaneous requests against a cap of 2 served **5** and refused 20. What survives is that under `READ COMMITTED` two statements overlapping from *below* the cap can each see the same pre-insert count and each insert, so the overshoot is now bounded by how many inserts genuinely overlap inside Postgres rather than by how many callers happened to be rendering. Closing it entirely would take `SERIALIZABLE` or a per-user lock on every AI call, which is not worth serializing the whole service for. This tightening covers the **single-actor** kinds only: a duel's player rows are unconditional (above), so the duel's per-user half is exactly as advisory as it always was — and its wider gap is the checked-at-create / billed-at-join one below, not this one.
- **The global half is a plain read and gates nothing.** Two simultaneous callers can each see a count one below the ceiling and both pass, so it overshoots by roughly the number of checks in flight; and the provider row a duel writes at `judging` is unconditional, because by then two people have drawn for ninety seconds and refusing would strand the round rather than prevent the call. The ceiling is *meant* to sit under the provider's own quota so that stays harmless — and on a free key it only half does, since a pass can be up to three requests (see **Defaults**). The honest backstop for the rest is `ErrQuotaExhausted` answering as a refusal rather than a crash (`JUDGE.md` §8.1).
- **The retries inside ONE judging pass are uncounted.** `Judge.Score` makes up to 3 attempts (`JUDGE.md` §7) and all of them bill as that pass's single provider row. The only way to count them would be to hand the frozen `Judge` contract a database dependency (`JUDGE.md` §2) — not a trade worth making for a number that is already an exact count of *passes*. What is no longer undercounted is the **re-fire**: a stuck-judging retry is a new pass and bills its own row, where up to three passes used to share one.
- **A duel's rows are invisible to a concurrent count until their transaction commits**, because each half is written inside the transaction that makes it true. Milliseconds, and the narrow price of "no round, no grant" and "no judging flip, no judge bill" being atomic.
...
- **A failed call is still counted.** The rows are written before the provider is called and are never rolled back, so a provider outage burns allowance. That is the conservative direction on purpose — a failure that costs nothing is exactly the one that drains a quota unseen. Its sharpest form: a duel that burns every retry and ends `aborted` (§4.1) bills a provider row *per pass* and produces no verdict at all, which is the right way round — the requests really were made.

Two inaccuracies the earlier shapes carried are **gone** rather than accepted: the global count could not see matches currently `drawing` (a duel is billed at the judging flip now, not derived from a lifecycle column), and a **forfeited or abandoned** round used to bill a provider request the judge never made.

Rows are swept on a retention horizon of a week (`aibudget.RunSweeper`): **one pass at boot, then hourly**. Nothing is ever *read* past the 24h window, but a week of history is what gives "why did the ceiling refuse me last Tuesday" an answer, at a few thousand rows. The boot pass is not impatience — nobody waits on a retention sweep — it is the host: a free-tier instance sleeps when idle and restarts on the next request, regularly more often than once an hour, so a sweep whose first pass was always an hour away would never have run at all.
...
> **Scope:** a deliberately small Elo sketch so a result *moves the needle*. Decay, provisional/placement handling and anti-abuse are out of scope. This pins just enough to apply a rating delta on every `done` match.
...
- **K-factor:** **K = 32** for v1 (a single flat K — simple and responsive).
...
Live realtime is a **delivery upgrade, not a second backend** — it pushes over WS exactly the transitions §3/§4 already define; it does not change the lifecycle, and it applies to every async match (there is no separate `matches.mode = 'live'` — the WS route is available on any match id the caller is a player in).
...
**Scored by a separate seam, not the `Judge` contract.** `docs/JUDGE.md` §2's `Judge` is frozen and inherently comparative (`prompt, pngA, pngB → scoreA, scoreB, winner, reason`) — the agreement with the external ML judge. Practice asks a different question, "how well does this ONE drawing depict the prompt?", so it is served by `judge.Critic` instead — ours, explicitly not something the external judge implements or is asked to. Full seam, including the `JUDGE_MODE` selection table: `docs/JUDGE.md` §8.2.
...
GET a prompt (never redacted — no opponent to be fair to)
  ...
  → POST the document
      budget check → authoritative render (off-client, §6)
        → bill the ledger (which may itself refuse — §4.3) → critique
