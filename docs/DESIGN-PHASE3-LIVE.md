<!-- DESIGN PROPOSAL — drafted 2026-07-11. Server-authoritative round deadline (forfeit-judge) + WS realtime, the back half of Phase 3. Defaults in §5 ACCEPTED by owner 2026-07-11 (see banner there). STATUS 2026-07-12: BOTH subsystems SHIPPED — the round-deadline subsystem (§2, `feat/round-deadline`) and the WS-realtime subsystem (§3, `feat/ws-realtime`) are live; decisions are folded into GAME.md / API.md / DECISIONS.md / NOTES.md. This document is RETAINED, not deleted, as the design rationale — code comments and docs/NOTES.md cite its section numbers (§2.x/§3.x) throughout. The body below is the historical design record as drafted/accepted; it is not re-verified against the shipped code line-by-line going forward — read GAME.md/API.md/NOTES.md + the code itself for current behavior. -->
