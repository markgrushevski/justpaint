...
- **Light-theme outline token fails WCAG non-text contrast** — `--ori-color-outline-light: hsl(28 22% 85%)` is only ~1.4:1 against the white light surface, below the WCAG 1.4.11 3:1 non-text (border) threshold. The **dark** outline was raised to `hsl(28 13% 46%)` (~3.4:1) on 2026-07-05, so the two themes are now **asymmetric** (dark passes, light doesn't). A conscious trade-off, not an oversight — deferred decision: either raise the light outline to clear 3:1 (which darkens the deliberately-soft warm borders/dividers/input outlines across the whole light theme — a visible aesthetic shift) **or** accept the soft borders as decorative (controls stay identifiable by fill/position). *When:* the next light-theme polish / a11y pass.
