...
- **Portfolio-grade auth surface (beyond the shared `AuthForm`)** (owner, 2026-07-11) — `feat/shared-auth-form` makes the sign-in form a shared component reachable inline on both `/play` and `/draw` (the auth *store*/`@core` client was already shared; only the *form UI* was trapped in `/draw`'s `SideMenu`). The **aspirational** upgrade is an auth experience that reads as senior product work. Candidates, roughly by impact: **(1) guest / anonymous quick-play** — jump straight into a duel on an ephemeral account, then optionally *claim* it (register to keep rating/history) — the biggest funnel win for a drawing-duel game and a strong product-thinking signal; **(2) a global, mode-agnostic auth affordance** — an app-shell "sign in" entry and/or a `/login` route + an accessible focus-trapped dialog (rebuild on `@oriui/headless` `useDialog`), so auth is a first-class cross-cutting surface, not a card per view; **(3) resume-after-auth** — sign in mid-flow and land back exactly where you were (the pending match), not a generic redirect; **(4) OAuth (GitHub/Google) and/or passkeys (WebAuthn)** — real-world identity integration, the most portfolio-impressive but the most infra (needs the `email` column split + provider wiring); **(5) form-feel polish** — inline field validation, password-strength meter, distinct error/loading/success states, "remember me". *Why:* auth is the first thing a portfolio reviewer touches; a frictionless guest→claim flow plus real OAuth/passkeys is a standout, and it directly serves the game (less sign-up friction = more duels). *When:* after the core `/play` loop lands; sequence (1)+(3) first (cheapest, highest funnel value), (4) when an email/identity flow is actually built. Pairs with the `email` column split (above) and the `useDialog` dialog rebuild (Frontend / build).
...
- **Cross-match rating lost-update (`users.rating`)** — `UpdateUserRating` writes `users.rating` as an absolute `SET`, with no row lock or CAS; correct only because a single match resolves exactly once, but a user seated in **two concurrently-resolving matches** can lose one rating update to the other (last write wins) — and nothing today stops a user from being in more than one active match. Switch to `rating = rating + delta` (atomic, no read-then-write) or serialize per user (`pg_advisory_xact_lock(hashtext(userID))`, the same idiom as the open-match dedupe gap above) before leaning on the ladder number. *When:* Phase 4 — forfeit (`feat/round-deadline`) and, later, live mode both raise the odds of actually hitting this. *(Full mechanism in `docs/NOTES.md`.)*
