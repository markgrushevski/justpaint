...
**Deliverables**
- [~] **`docs/GAME.md` realized** — match lifecycle in Go. **Front half landed** (`feat/game-matches`): create/auto-join → `open`/`drawing`; canonical game canvas (1080²) echoed to the client. **Remaining:** submit → judging → done/abandoned (needs the render worker below).
...
- [~] **Matches** — `matches` + `match_players` wired in `internal/game`. **Landed:** `POST /api/matches` open-pool create/auto-join (tx + `FOR UPDATE SKIP LOCKED`, no double-seat — verified live), `GET /api/matches/{id}` with ownership hidden as 404 and the two visibility redactions (prompt text while `open`, opponent `drawingId` until `done`); state machine `open → drawing`. **Remaining:** `submit`/`abandon` transitions (`drawing → judging → done/abandoned`), `match_players.drawing_id`/`submitted_at` stamping — next slice, with the render worker.
