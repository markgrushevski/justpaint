...
- [ ] **UI/UX pass (`/draw` + app shell) — gates `/play`** (owner directive 2026-07-04, `DECISIONS.md`): proper oriui layout/spacing on the toolbar/panels; the legacy **slide-in side menu** returns (hidden by default, toggle button; auth + profile inside); canvas interaction fixes — no drawing outside the document area, immediate eraser feedback; active-tool/disabled states; responsive check across oriui breakpoints.
