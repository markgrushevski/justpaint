...
**Deliverables**
- [ ] **Monorepo skeleton** — create the target layout (`packages/document`, `packages/editor`, `apps/web`, `server/`); migrate `client/` into `apps/web` as-is (refactor is Phase 2).
- [ ] **`packages/document` v1** — canonical TS types (§3–§5 of `DOCUMENT-FORMAT.md`), `parseDocument`/`serializeDocument`, and the `renderToPNG` seam (browser path via Konva first; Node worker can follow). Pin the perfect-freehand version + the brush-subset/fit/fill render constants (DOCUMENT-FORMAT §5.3/§6/§10).
...
- [ ] **Drawings CRUD** — create/read/update/list/delete; **Go-side document validation at the write edge** (§7: size guard via `http.MaxBytesReader`, typed decode with `Stroke.UnmarshalJSON`, nullable Color as `*Color`, invariants incl. point-tuple arity + `strokeWidth>0`-when-stroke-present, total-points DoS cap). Reject malformed/oversized docs.
- [ ] **Minimal Konva editor** — full tool set against the schema (pen/brush, eraser, line, rect, ellipse, triangle-as-polygon); pointer input captured in **logical/stage coords** (`getRelativePointerPosition`, never `pageX/offsetLeft`); save/load round-trips a document through the Go API.
- [ ] **Tests** — table-driven Go tests for the validator (valid + each rejection path); document round-trip tests in TS.
...
**Deliverables**
- [ ] **`packages/editor`** — extract tools, layers, history, export from `apps/web`; consumed by both modes. No Vue/router/API-client dependency (it depends only on `packages/document` + Konva + perfect-freehand — ARCHITECTURE §3).
