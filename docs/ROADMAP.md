...
**Deliverables**
- [x] **`docs/GAME.md` realized** — the **full async match lifecycle** runs in Go (`feat/game-matches` + `feat/game-submit`): `open → drawing → judging → done`, create/auto-join, submit, out-of-band judging, result — verified live end-to-end. (`abandoned` + the live back-half remain.)
...
- [~] **Submit** — the submit path + trust boundary **seam** landed. The server persists the vector doc, renders the judged raster **off the client behind a `render.Renderer` interface**, and scores it — client PNGs never touch the judge. **Remaining:** the *real* renderer — today a deterministic **`render.StubRenderer`** (ink ∝ stroke count) runs the loop; the pixel-authoritative **Node Konva + perfect-freehand worker** (sharing `packages/document`) swaps in behind `Renderer` with no loop change.
...
- [ ] **Object storage seam** — rendered judged PNGs (and thumbnails) to object storage (URLs handed to the judge / stored in `thumbnail_url` / `judgedImageUrl`); local/dev stub acceptable first. **Now the gating item for the result reveal** — `judgedImageUrl` is `null` until this + the real render land (the opponent's canvas can't be shown via the ownership-scoped drawings route).
...
**Exit criteria**
- Two users can play an **async duel** end-to-end: same prompt → both draw → submit → fake judge scores → winner + reason shown — with the scored image rendered authoritatively server-side from the vector document. **✅ Loop met live** (`feat/game-submit`): submit → out-of-band judging → winner/scores/reason/Elo. **Caveat:** the raster comes from `render.StubRenderer` (loop-proving), not yet the pixel-authoritative Node worker — that swap is the one remaining piece of "rendered authoritatively".
- Swapping the fake judge for the collaborator's HTTP judge requires no schema/loop change (only the `Judge` impl). **✅ Structurally met** — the game loop depends only on the `Judge` interface; the render worker is the **same seam pattern** (`Renderer` interface), so both real impls drop in without touching the loop.
- (Live half) two users see each other's match state in realtime over WS. *(not started)*
