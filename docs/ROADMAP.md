...
- [ ] **UI/UX pass (`/draw` + app shell) — gates `/play`** (owner directive + pinned choices 2026-07-04, `DECISIONS.md`): **floating bottom toolbar** (tldraw-style; the editor shell is **shared with `/play`** — one design, game chrome layers on top); the legacy **slide-in side menu** returns (hidden by default, toggle button; **auth + profile inside — replaces the top SessionBar**); canvas interaction fixes — **layers clipped to the document, gestures starting outside ignored** (a stroke may run past the edge, visually clipped), **immediate eraser feedback** (in-flight stroke composited on the target layer); **full hotkey set** (tools, undo/redo, zoom, save) + a shortcuts cheat-sheet; responsive + dark check across oriui breakpoints.
