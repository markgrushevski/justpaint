...
**Deliverables**
- [~] **Monorepo skeleton** — npm-workspace root in place (`package.json` workspaces `packages/*`+`apps/*`, shared strict `tsconfig.base.json`) with `packages/document` as the first member; `server/` already at the target path. **Remaining:** migrate `client/` → `apps/web` as-is (next step; refactor is Phase 2).
...
- [~] **Tests** — Go validator: table-driven tests done (a valid full document + every rejection path + union decode, `internal/document`). **TS `packages/document` tests done** (40: validate table mirrored from Go, serialize/parse round-trip + write-precision, fit transform). Editor tests come with the Konva editor.
