...
- [~] **Minimal Konva editor** — built as `packages/editor` (`@justpaint/editor`): full tool set against the schema (pen/brush, eraser, line, rect, ellipse, triangle-as-polygon) as **pure `buildStroke` tools** (no Konva/state → unit-testable); `toKonva` projection (§6) + `renderToPNG` (§10 fit + per-layer isolation, browser path); an `Editor` controller capturing input in **logical coords** (`getRelativePointerPosition`, never `pageX/offsetLeft`). 14 Vitest tests + typecheck + build green; `FREEHAND_VERSION` pinned to the resolved perfect-freehand 1.2.3. **Remaining:** wire the editor into `apps/web` and round-trip save/load through the Go API.
