...
- [x] **`/play` page** — create/auto-join, draw, submit, poll the verdict, see result + reason + Elo, **live against `/api/matches`** (`feat/play-api-loop`, 2026-07-09). Reuses `packages/editor` on the shared `EditorShell`; a typed `core/api/matches.ts` client (1:1 with the Go DTOs) + TanStack `useCreateMatch`/`useSubmitMatch` mutations; one self-rescheduling poll loop (roster → verdict) stands in until the live WS push. Anonymous visitors get a sign-in card. **Gated (own lines below):** the opponent's canvas on the reveal (`judgedImageUrl` null until the object-storage seam) and a server-authoritative round deadline (the client timer is a soft UX pressure only).
