...
>
> **Amended 2026-09-20 (the real impl): a prompt finally becomes shapes, and the vendor is Google, not Anthropic.** `ASSIST_MODE=gemini` selects `GeminiAssist` (`server/internal/assist/gemini.go`), which reaches the same API — and the same key, quota and HTTP client — as the judge, the critic and the guesser (`judge.GeminiClient`, `JUDGE.md` §8.1). §3.2 and §3.3 now describe **that** impl and are no longer a plan. `AnthropicAssist` is **gone**: the Anthropic SDK was never added to `go.mod`, and the scaffold, its mode and its two knobs (`ASSIST_MODEL`, `ANTHROPIC_API_KEY`) were deleted later the same day rather than left to rot — `ASSIST_MODE=anthropic` is now a boot error that names `gemini`, deliberately, since falling back to `fake` would boot green and then answer every prompt with the same canned house. The Op contract (§2), the doc summary (§4) and the client (§5) are unchanged — the real impl produces exactly the batch the fake did, which is what made the swap a config change.
...
There is no third mode. `anthropic` was one until 2026-09-20 — the Phase A scaffold that never made a call — and it is now **refused by name at boot**, not silently downgraded to `fake`: an environment still carrying it is out of date rather than mistyping, and a fake that boots green while answering every prompt with the same canned house is a dead feature wearing a live one's face.
...
- **The model is the same knob every Gemini seam reads**: `GEMINI_MODEL` (default `gemini-3.6-flash`), overridden for this kind alone by `AI_MODEL_PER_KIND=assist=…`; `GEMINI_BASE_URL` for the API root. Assist gets **no model knob of its own** — there was an `ASSIST_MODEL`, and it was deleted with the `anthropic` mode it belonged to — because the quota assist spends is counted per provider **and model** (`GAME.md` §4.3), so the model id and the budget key have to be the same fact, and a second knob could only ever disagree with it.
...
- **The real impl (2026-09-20):** `GeminiAssist` behind `ASSIST_MODE=gemini` — same ops, same client, same UX, a prompt that is finally read (§3.2). `AnthropicAssist`, its mode and its knobs were **deleted** the same day rather than left as dead weight; the collaborator's ML may still take this seam later, which is what the interface is for.
