...
The bucket bounds the **rate** — how fast one user may ask — and it lives *in this process*, so the host resets it on every deploy and every wake from idle. That means it was never a ceiling at all: it could be emptied, and then handed back in full, as often as the instance restarted. Since **2026-09-20** assist therefore also sits under the **daily AI-call budget** (`docs/GAME.md` §4.3, `server/internal/aibudget`), which bounds the daily **quota**, lives in Postgres, and so actually holds: kind `assist`, default **40 calls per player per rolling 24h**, plus the per-provider global ceiling that every AI feature shares. It is enforced only when the impl really calls a provider — and the **impl is asked**, never inferred from the mode (`assist.CallsProvider`). Today it answers *false* for both: `FakeAssist` is offline by construction, and `AnthropicAssist` is still the Phase A scaffold that returns an error with no network I/O at all. So assist is currently **unbudgeted**, `ASSIST_MODE=anthropic` says so at boot (`Warn`), and the `anthropic` provider is switched on by the same change that wires the SDK and flips `CallsProvider` to true. This matters because the mode used to be read as the answer: `ASSIST_MODE=anthropic` was billing a ledger row for a call that never happened, and then answering `500` (`docs/DECISIONS.md` 2026-09-20 review entry).
