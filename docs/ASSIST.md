...
>
> **Amended 2026-09-20 (the real impl): a prompt finally becomes shapes, and the vendor is Google, not Anthropic.** `ASSIST_MODE=gemini` selects `GeminiAssist` (`server/internal/assist/gemini.go`), which reaches the same API — and the same key, quota and HTTP client — as the judge, the critic and the guesser (`judge.GeminiClient`, `JUDGE.md` §8.1). §3.2 and §3.3 now describe **that** impl and are no longer a plan. `AnthropicAssist` is **superseded and is not coming**: the Anthropic SDK was never added to `go.mod` and will not be, the mode is kept only so an environment that already sets it still boots, and `ASSIST_MODEL` / `ANTHROPIC_API_KEY` are vestigial knobs. The Op contract (§2), the doc summary (§4) and the client (§5) are unchanged — the real impl produces exactly the batch the fake did, which is what made the swap a config change.
...
| `anthropic` | `AnthropicAssist` | The Phase A scaffold, **superseded and not coming**. It boots (given `ANTHROPIC_API_KEY`), calls nobody, bills nothing, warns at boot, and fails every request with *"assist: anthropic impl not built"*. Kept only so a deployment that already names it still starts. |
...
- **The model is the same knob every Gemini seam reads**: `GEMINI_MODEL` (default `gemini-3.6-flash`), overridden for this kind alone by `AI_MODEL_PER_KIND=assist=…`; `GEMINI_BASE_URL` for the API root. Assist gets **no model knob of its own** — `ASSIST_MODEL` belongs to the dead `anthropic` mode — because the quota it spends is counted per provider **and model** (`GAME.md` §4.3), so the model id and the budget key have to be the same fact.
...
- **Phase A (MVP, 2026-07-13):** v1 ops (`add_layer`, `add_stroke`), ghost preview + accept/reject, fake `Assist` impl (live) + `AnthropicAssist` (config-gated scaffold only, no live SDK call), the prompt panel in `/draw`.
- **The real impl (2026-09-20):** `GeminiAssist` behind `ASSIST_MODE=gemini` — same ops, same client, same UX, a prompt that is finally read (§3.2). `AnthropicAssist` is superseded and its SDK is not being added; the collaborator's ML may still take this seam later, which is what the interface is for.
