...
> **The contract for the external ML judge.** The judge is built externally, as its own project — **we never build the ML.** We own only this contract and a fake implementation. This document is the single source of truth for the judge's exact shape: the request/response, the **positional `winner` semantics**, tie rule, the judged-raster spec (size + background), and the transport. If code or another doc disagrees with this file, **this file wins** for everything judge-related.
>
> **Ownership note.** `docs/ARCHITECTURE.md` §5, `docs/DOCUMENT-FORMAT.md` §10, and `docs/GAME.md` all *defer* to this doc for the `winner` representation, tie semantics, raster size, and background. This doc, in turn, defers the trust boundary / render pipeline to `DOCUMENT-FORMAT.md` §10 and the match lifecycle / A·B→player resolution to `GAME.md`. `docs/GAME.md`'s practice section defers the `Critic` seam (§8.2) to here in exactly the same way, and `docs/API.md` §13 defers the `Guesser` seam (§8.3) — related but explicitly separate contracts, never a widening of §1–§11.
>
> **Status:** v1 contract, frozen. `FakeJudge` ships as the dev/CI default; `HTTPJudge` (§7) is built against this contract and waiting for the external judge service to exist; `GeminiJudge` (§8.1) is a real interim judge — a vision LLM scoring both rasters — so the product isn't stuck on a fake verdict while the external ML is built. Two local seams live alongside it and are explicitly NOT part of this frozen contract: the `Critic` (§8.2) serves single-player practice, which needs a different, non-comparative question answered, and the `Guesser` (§8.3) serves the `/draw` guess button, which has no prompt to compare anything against at all. Greenfield. The judge service can integrate against this contract alone — it never reads our other docs, never parses our document schema, never runs `getStroke`.
...
The `HTTPJudge` (§7) calls the external judge service over HTTP/JSON. This section is the wire agreement.
...
**Image delivery — two supported modes (the judge service picks one; we configure to match):**

- **Inline base64 (default for v1):** `imageA`/`imageB` are base64-encoded PNG bytes as JSON strings (optionally a `data:image/png;base64,` prefix). Simplest to integrate; at 1024² a PNG payload is small. This is the v1 default because it needs no shared object storage between us and the judge service.
...
**`HTTPJudge` is built** (`internal/judge/http.go`) — the transport to the external judge service. Its behavior:
- **Timeout is PER ATTEMPT, not per call.** `JUDGE_TIMEOUT` (default **10s**) bounds one HTTP round trip, layered as its own `context.WithTimeout` under the caller's `ctx` on every attempt — a single budget shared across attempts would make "retry on timeout" dead code, since the first timeout would consume it. The judge, the critic and the guesser share it, because all three ask one short question and get a handful of scalars back. **Assist does not**: it has `ASSIST_TIMEOUT` (default 60s), because composing a picture is a different length of answer and because widening the shared knob would break the `3 × JUDGE_TIMEOUT < game.JudgePassBudget` envelope that boot warns about (`ASSIST.md` §3.2).
...
- **A `200` that fails `Result.Validate()` is a contract violation, not a verdict** — wrapped in `ErrInvalidResult` and, deliberately, **not retried**: the judge service is pure, so a same-content retry would earn back the identical broken body. A `200` whose body isn't even JSON is a separate, equally non-retryable, error class.
...
- **A daily call budget gates the judge from outside this interface entirely.** Before `Judge.Score` is ever reachable, `internal/game` refuses a new duel once a rolling-24h per-player cap on the `duel` kind, or the whole daily budget of the provider backing the judge, is spent (`AI_DAILY_GLOBAL` / `AI_DAILY_PER_USER`, with `JUDGE_DAILY_BUDGET` / `JUDGE_DAILY_PER_USER` kept alive as aliases; enforced only when the configured impl is real, so `JUDGE_MODE=fake` spends nothing) — a control this interface carries no notion of, deliberately: the scarcity being managed is an external quota that is ours to protect, not part of the judge's contract. `JUDGE_MODE=http` still gets a ceiling, under provider `collaborator`: that quota is not ours and we cannot see it, which is exactly why we keep one under it rather than waiting to be told we exceeded it. Worth knowing here: a duel's two ledger halves are billed at two moments — the **players** when their round starts, the **provider** once per entry into `judging`, which a stuck-judging re-fire performs again and a forfeited or abandoned round never performs at all. So a re-fire *is* counted; what stays uncounted is only this section's 3 attempts **inside** one pass, deliberately, because counting them would give this frozen interface a database dependency it must not have. Full rule: `docs/GAME.md` §4.3; why: `docs/DECISIONS.md` 2026-09-19 and 2026-09-20.
...
`FakeJudge` runs **in-process** (no HTTP), needs no network, and is the default for dev and CI.

## 8.1. `GeminiJudge` — an interim real judge, while the external ML is built

`FakeJudge` (§8) proves the loop but never reads the prompt; `HTTPJudge` (§7) has nothing to call until the external judge service exists. `GeminiJudge` (`internal/judge/gemini.go`, `JUDGE_MODE=gemini`) closes that gap: it sends a vision model both judged rasters in one request and takes its structured JSON output as the §2 `Result`, so the product ships a verdict that actually looks at the prompt and the pictures instead of an ink-coverage proxy, for as long as the external ML is still being built.
...
- **The transport is one type, now shared by FOUR seams and EXPORTED for the fourth.** `GeminiClient` (`internal/judge/gemini.go`) owns the endpoint, the credential, the per-attempt deadline and the §7 retry policy, and knows nothing about what is being asked: `GeminiJudge`, `GeminiCritic` (§8.2) and `GeminiGuesser` (§8.3) embed it, and so does `assist.GeminiAssist` (`ASSIST.md` §3.2) from **outside this package**, through the exported `NewGeminiClient` / `GenerateJSON` / `GeminiSchema` / `GeminiFinishTruncated`. The alternative was a second copy of the same plumbing, which would have drifted from this one the first time Google changed anything. What `internal/assist` must **not** take from this package is a `Judge`, a `Critic` or the §2 contract — only the wire. The honest home for a provider client is a package neither judging nor assisting owns; it lives here because that move is a file rename, and because everything in it is transport with no judgement in it. The `label` each caller passes (`"judge: gemini critic"`, `"assist: gemini"`) is what keeps four seams' failures distinguishable in one log.
...
- **`ErrQuotaExhausted` wraps HTTP 429 and is never retried** — a daily quota does not refill in 250ms, so retrying would just spend two more log lines waiting for the same failure. The wrapped error carries Google's own error message verbatim, so a log line can tell a spent daily budget apart from an ordinary per-minute rate limit. It reaches a player differently on each route: the three **synchronous** ones (`/api/practice` §8.2, `/api/guess` §8.3, and since 2026-09-20 `/api/assist/ops` too — `ASSIST.md` §3.3) answer it with the budget's own global `429` rather than an opaque `500` — the provider ran out before our own ceiling did, which is the same news from the other end, and a `500` would invite a retry that cannot succeed until the provider's window rolls (`API.md` §10/§12/§13). Assist could not do this until it had an impl that reached a provider at all. A **duel** has no request left to answer, so it is logged by name and the round is left to the stuck-judging sweep, which re-fires it and then closes it out as `aborted`.
- **One deliberate asymmetry, worth calling out on its own: an over-long `reason` is CLAMPED here, but REJECTED by `HTTPJudge`.** This impl owns the model it prompts, and verbosity is a failure mode we invited by asking for prose in the first place — discarding a duel both players finished, over a rationale that ran forty characters long, is the worse outcome. A peer service breaking the same §2 cap is different: it is a contract violation, and silently repairing it would hide a real break in someone else's system. Scores and `winner` — the parts that actually decide the duel — are validated strictly in both, with no clamping involved.
...
- **`GEMINI_MODEL` is the DEFAULT for every AI kind, not the judge's own setting — `AI_MODEL_PER_KIND` overrides it per kind.** `AI_MODEL_PER_KIND="duel=…,guess=…"` (the same `kind=value` comma list `AI_DAILY_PER_USER` takes, through the same parser) pins one kind to one model; an unknown kind name is a boot error, and the resolved model per kind is **logged at boot**, because the failure mode of a per-kind override is silent — a kind whose override never took keeps working perfectly on the default, and the only symptom is a bill or a quality level nobody chose. The knob exists because the jobs differ: the duel judge compares two drawings and its number feeds Elo, so it should be the steadiest; the critic scores one drawing against a prompt; the guesser only names what it sees and its mistakes cost nothing; assist composes shapes. It has a budget consequence that is not a side effect but half the point — Google meters its free tier **per project per model**, so a kind on its own model gets its own quota pool and its own ceiling (`google:<model>`, `GAME.md` §4.3).
...
Everything from §1 through §8.1 — the `Judge` interface, `Request`/`Result`, the positional `winner`, the judged-raster spec — is the frozen agreement with the external ML judge (§1). **`Critic` (`internal/judge/critic.go`) is a second, LOCAL seam, ours to move, that the external judge does not implement and is not asked to:**

```go
// internal/judge/critic.go — OURS, not part of the external judge's contract above.
...
| `http` | `HTTPJudge` — the external judge service | **unconfigured.** The external judge service answers a comparative question and has no critique endpoint — it was never asked to build one. `internal/practice` gets a `nil` `Critic`, and every `/api/practice` call refuses with `500` rather than silently falling back to `FakeCritic`: a fabricated score presented as a real critique is a lie the player cannot detect, and that is worse than an honest refusal. `docs/API.md` §12 owns the exact response. |
| `gemini` | `GeminiJudge` — one `generateContent` call, both rasters | `GeminiCritic` — one `generateContent` call, the one raster. Shares `GeminiClient` with `GeminiJudge` (§8.1): same endpoint, same header-based key (never `?key=`), same §7 retry policy, same `ErrQuotaExhausted` — a practice run and a duel fail, and get fixed, the same way. Its model is `GEMINI_MODEL` unless `AI_MODEL_PER_KIND` names one for `practice`. |
...
Full HTTP edge (routes, every error code, the exact wording of the "unconfigured" `500`): `docs/API.md` §12. Why practice is not a `matches` row, why it draws on the shared daily AI-call budget (its own per-player allowance, the same per-provider ceiling), why it is not rated: `docs/GAME.md` §10. The decisions and their reasoning: `docs/DECISIONS.md` 2026-09-20.

## 8.3. The `Guesser` seam — "what did I draw?", a THIRD seam and the second local one

The button on `/draw` that asks the AI what the canvas depicts (`docs/API.md` §13) runs on a third interface in `internal/judge`, and the second one the external judge neither implements nor is asked to:

```go
// internal/judge/guess.go — OURS, like Critic, and for the same reason.
type Guesser interface {
    Guess(ctx context.Context, img []byte) (Guess, error)
}

type Guess struct {
    Label        string   // a short noun phrase, ≤80 runes — "a cat wearing a hat"
    Confidence   float64  // [0,1] — NOT a Critique's Score; see below
    Alternatives []string // 0-2 runner-ups, genuinely different subjects
}
```

**Why it is not a widened `Judge`, and — the closer call — why it is not a `Critique`.** `Judge` is frozen (§1, §10), takes two images, and answers a comparative question; that one hardly needed arguing. The seam it really had to be separated from is `Critic` (§8.2), which also takes one image and also returns a bounded number. What makes a guess a different question is **what is missing from it**: a `Critique` scores ONE drawing against the prompt it was drawn for, and on `/draw` there is no prompt, because nobody supplied an answer. The player opened a blank canvas, drew whatever they felt like, and asked *"what is this?"* — so there is nothing to score against, and therefore no score. `Critic` answers "how well does this depict X"; `Guesser` answers "what is this". Reusing `Critique` would have meant inventing a prompt to compare against, which is precisely the thing nobody did.

**`Confidence` and `Score` share a scale and mean different things.** A score says how well a drawing matched an answer **we** gave the player; a confidence says how sure the model is of an answer **it made up itself**. They are not comparable and must never be shown side by side as if they were — which is why the `/draw` card renders the confidence as a phrase and keeps the number to itself.

**`Guess.Validate()` mirrors `Critique.Validate()`'s strictness, with one place it is stricter.** Confidence finite and in `[0,1]`; at most two alternatives; no alternative blank (a hole in a list the client renders as rows) or over the label cap. The stricter part is the label itself: **empty feedback is legal on a `Critique`** — it is commentary on a score that stands without it — while an **empty label is not**, because a guess with no label is nothing at all; there is no other field for the answer to live in. The 80-rune cap is deliberately short, a sixth of the critic's feedback cap, because the shortness *is* the product: a model that wants to explain itself would otherwise turn the button into an essay generator.

**`JUDGE_MODE` selects the guesser too, so all three vision seams move together:**

| `JUDGE_MODE` | `Guesser` (the `/draw` button) |
|---|---|
| `fake` (default) | `FakeGuesser` — ink coverage again, and the **label says so in as many words** ("a few marks (the fake guesser sees ink, not meaning)"). That sentence is the point of the type: "a cat wearing a hat" from something that only counted dark pixels is a lie a player cannot detect. Never any alternatives — a guesser that never looked has no second opinion, because it never had a first one. |
| `http` | **unconfigured.** The external judge service implements the two-image §2 contract and has no endpoint that names one drawing — it was never asked to build one. `internal/guess` gets a `nil` `Guesser` and every `/api/guess` call refuses with a `500` that names the cause, deliberately **not** falling back to `FakeGuesser`: a fabricated guess is a *more* convincing lie than a fabricated score, because a wrong guess is funny, so it reads as the feature working rather than as the feature being off. |
| `gemini` | `GeminiGuesser` — one `generateContent` call, one raster. Shares `GeminiClient` with `GeminiJudge` and `GeminiCritic` (§8.1): same endpoint, same header-based key, same §7 retry policy, same `ErrQuotaExhausted`, same `JUDGE_TIMEOUT` — a guess, a practice run and a duel fail the same way and are fixed the same way. The guess button gets **no key of its own**, and no model knob of its own either beyond the one every kind has: `GEMINI_MODEL` unless `AI_MODEL_PER_KIND=guess=…` gives it a cheaper model, which is exactly the kind it was added for — the guesser's mistakes cost nothing. Same API, and the same quota whenever it is the same model. |

**The instruction asks for nerve, not for a scale.** There is no answer key here and no verdict to justify, so on this feature the system instruction *is* the feature: commit to a subject whenever the marks support one (an honest specific guess is the entire point; "a drawing" is right only when there is genuinely nothing else to say), mean the confidence rather than inflating or deflating it for tone, and never mock the picture or give drawing advice — the person reading the answer is the person who drew it.

**The runner-ups are two named optional fields, not an array — on purpose.** `alternative1` / `alternative2` in the response schema, collapsed into the 0-2 `Alternatives` slice on the way out. Our `GeminiSchema` models only the OpenAPI subset we actually use, and it still has **no `minItems`/`maxItems`** — the REST Schema types those as int64 formatted *as a string* (`"3"`, not `3`), which is cheap to get wrong and costs a `400` on every call until someone notices, so they wait for a live call to prove the spelling. (It does now carry `items`, added for the shape list in `ASSIST.md` §3.2, which is a list however it is spelled.) An array here would therefore reach the model as an *unbounded* list and the 0-2 arity would become something we trim after the fact. Two named fields pin the arity **in the schema, where the model is choosing**, and let each slot's description say what it is for. Two is a tiny fixed arity, not a list. Only `label` and `confidence` are required: requiring the alternatives would make the model invent doubt to fill them. On the way back, blanks are dropped (over a structured-output wire "optional" means an empty string far more often than an absent key) and so are restatements of the label or of each other — "a cat" listed twice is noise, not a second opinion.

**The raster is rendered server-side, and here that matters more than anywhere else in this doc.** Two reasons, and the second is specific to this route. First the standing trust boundary: anything judged, scored or sent onward is derived server-side from the validated vector document, never from a picture the client supplied (§5, `DOCUMENT-FORMAT.md` §10, `GAME.md` §6). Second, and sharper — **accepting a client PNG here would turn the route into a one-line pipe to a third party under OUR API key.** Any authenticated caller could `POST` a photo, a screenshot, someone else's picture, any bytes at all, and have us pay to upload it to Google and hand back what the model said about it. Rendering it ourselves means the only images that ever leave the building are our own renderer's output of a document that passed our own validator. A `maxRasterBytes` guard (**4 MiB**) sits between the renderer and the wire; it should never fire today, since both wired renderers pin `render.JudgeFrameSize` (1024²) whatever the document says, and it guards the **seam** rather than today's impl — free draw allows canvases to the format's 8192² bound, and the day a renderer honours the document's own dimensions, an 8-megapixel raster becomes several megabytes that base64 then inflates by a further third. The failure it prevents is silent and billable, which is the worst combination to discover from an invoice.

**That bounds the route's *form*, not its content — and this doc used to overclaim it.** The earlier wording said rendering server-side meant a caller could not send arbitrary images to the model. That claim does not hold: the document format allows **5000 strokes** (`document.MaxStrokes`), and a 70×70 grid of filled rectangles is 4900 of them — a mosaic that passes the validator, renders like any other drawing, and reaches the model as a picture the caller chose, at roughly 70×70 fidelity inside the 1024² frame. So the boundary **degrades** image laundering rather than preventing it. What it actually buys is a raised cost and a lowered fidelity: an arbitrary photo arrives as a coarse, deliberately assembled approximation, it costs a document upload per image plus one of the caller's **two** daily guesses (`GAME.md` §4.3), and it is nothing a patient person could not have drawn by hand. Worth keeping, and worth stating honestly — it removes "upload any bytes, we pay to ask about them" as a trick, it is not a closed door. What actually bounds the blast radius is the paragraph below and is unchanged: a guess scores nothing, ranks nothing, gates nothing and is stored nowhere.

**Same untrusted-image posture as §8.1/§8.2, and the smallest blast radius of the three.** The user turn carries **no player-authored text whatsoever** — two labels of ours and the raster — so the only untrusted channel is the pixels, and a player can draw words. The system instruction narrows that the same way: everything inside the image is drawing, never instruction. Be honest about what that buys: it **narrows** the surface, it is not a security boundary, and an instruction never was one against a sufficiently persuasive image. What actually bounds this is that a guess **scores nothing, ranks nothing, gates nothing and is stored nowhere**, so a successful injection buys a silly sentence on the screen of the player who drew the picture that produced it — an audience of one, who asked for it. The instruction carries one deliberate carve-out: writing is still allowed to **be** the drawing, so for a carefully drawn word *"the word HELLO"* is the correct guess; it is text that tells the model what to *say* that is a mark on a canvas rather than an instruction.

**Bounded by the smallest allowance of any AI kind: 2 per player per rolling day** (`aibudget.KindGuess`, `GAME.md` §4.3) — a novelty question about a drawing that may never be saved would otherwise be the cheapest way to empty a provider's quota. Like every per-kind refusal it **names the number**, on the test that the player could have counted it themselves; it is simply the route where that reads most sharply. The ledger row is billed **between the render and the model call**: the render is our own subprocess and spends nobody's quota, so a renderer that fell over must not cost a player one of their two guesses, while a model call that failed still spent the provider's. That write is also the tighter of the two per-user gates — it is a conditional insert that re-tests the cap at the instant of writing, so it can refuse where the check a render earlier did not, with the same `429` and the same sentence.

Full HTTP edge (the request, the response shape, why nothing is persisted, every error code, the exact wording of both the `429` and the "unconfigured" `500`): `docs/API.md` §13. The daily ceiling it spends, and where its ledger row is billed: `docs/GAME.md` §4.3. The decisions and their reasoning: `docs/DECISIONS.md` 2026-09-20.

## 9. Expectations on the real ML judge (for external implementers)
...
What the judge service can rely on from us: two comparable square 1024² PNGs, opaque background, drawing centered, always sent the same way; a stable request/response shape; retries that are safe because scoring is pure.
...
- **Breaking changes bump it:** changing the score range/meaning, the `winner` enum, the raster size/background, the image-delivery contract, or making `reason` structured. On a bump we run both versions behind config until cutover, coordinated with whoever runs the external judge.
...
- **The ML itself** → the external judge service. We never build it; we only define this contract and ship the `FakeJudge`.
