...
- **`ErrQuotaExhausted` wraps HTTP 429 and is never retried** — a daily quota does not refill in 250ms, so retrying would just spend two more log lines waiting for the same failure. The wrapped error carries Google's own error message verbatim, so a log line can tell a spent daily budget apart from an ordinary per-minute rate limit. It reaches a player differently on each route: the two **synchronous** ones (`/api/practice` §8.2, `/api/guess` §8.3) answer it with the budget's own global `429` rather than an opaque `500` — the provider ran out before our own ceiling did, which is the same news from the other end, and a `500` would invite a retry that cannot succeed until the provider's window rolls (`API.md` §12/§13). A **duel** has no request left to answer, so it is logged by name and the round is left to the stuck-judging sweep, which re-fires it and then closes it out as `aborted`.
...
| `gemini` | `GeminiGuesser` — one `generateContent` call, one raster. Shares `geminiClient` with `GeminiJudge` and `GeminiCritic`: same endpoint, same header-based key, same §7 retry policy, same `ErrQuotaExhausted`, same `GEMINI_MODEL`/`JUDGE_TIMEOUT` — a guess, a practice run and a duel fail the same way and are fixed the same way. The guess button gets **no model or key knob of its own**; it is the same quota on the same API. |
