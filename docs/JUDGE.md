...
- **A daily call budget gates the judge from outside this interface entirely.** Before `Judge.Score` is ever reachable, `internal/game` refuses a new duel once a rolling-24h per-player cap on the `duel` kind, or the whole daily budget of the provider backing the judge, is spent (`AI_DAILY_GLOBAL` / `AI_DAILY_PER_USER`, with `JUDGE_DAILY_BUDGET` / `JUDGE_DAILY_PER_USER` kept alive as aliases; enforced only when the configured impl is real, so `JUDGE_MODE=fake` spends nothing) — a control this interface carries no notion of, deliberately: the scarcity being managed is an external quota that is ours to protect, not part of the collaborator's contract. `JUDGE_MODE=http` still gets a ceiling, under provider `collaborator`: his quota is not ours and we cannot see it, which is exactly why we keep one under it rather than waiting to be told we exceeded it. Worth knowing here: the budget bills **one row per duel**, at the instant the round starts — so this section's 3 attempts, and a stuck-judging re-fire, cost the provider more requests than the ledger records. Full rule: `docs/GAME.md` §4.3; why: `docs/DECISIONS.md` 2026-09-19 and 2026-09-20.
...
**Bounded by the smallest allowance of any AI kind: 2 per player per rolling day** (`aibudget.KindGuess`, `GAME.md` §4.3) — a novelty question about a drawing that may never be saved would otherwise be the cheapest way to empty a provider's quota. It is also the one per-kind refusal whose message **names the number**, on the test that the player could have counted it themselves. The ledger row is billed **between the render and the model call**: the render is our own subprocess and spends nobody's quota, so a renderer that fell over must not cost a player one of their two guesses, while a model call that failed still spent the provider's.
