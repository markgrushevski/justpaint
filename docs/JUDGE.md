...
>
> **Status:** v1 contract, frozen. `FakeJudge` ships as the dev/CI default; `HTTPJudge` (§7) is built against this contract and waiting for the collaborator's service to exist; `GeminiJudge` (§8.1) is a real interim judge — a vision LLM scoring both rasters — so the product isn't stuck on a fake verdict while the collaborator's ML is built. Two local seams live alongside it and are explicitly NOT part of this frozen contract: the `Critic` (§8.2) serves single-player practice, which needs a different, non-comparative question answered, and the `Guesser` (§8.3) serves the `/draw` guess button, which has no prompt to compare anything against at all. Greenfield. The collaborator can integrate against this alone — he never reads our other docs, never parses our document schema, never runs `getStroke`.
...
- **A daily call budget gates the judge from outside this interface entirely.** Before `Judge.Score` is ever reachable, `internal/game` refuses a new duel once a rolling-24h per-player cap on the `duel` kind, or the whole daily budget of the provider backing the judge, is spent (`AI_DAILY_GLOBAL` / `AI_DAILY_PER_USER`, with `JUDGE_DAILY_BUDGET` / `JUDGE_DAILY_PER_USER` kept alive as aliases; enforced only when the configured impl is real, so `JUDGE_MODE=fake` spends nothing) — a control this interface carries no notion of, deliberately: the scarcity being managed is an external quota that is ours to protect, not part of the collaborator's contract. `JUDGE_MODE=http` still gets a ceiling, under provider `collaborator`: his quota is not ours and we cannot see it, which is exactly why we keep one under it rather than waiting to be told we exceeded it. Worth knowing here: a duel's two ledger halves are billed at two moments — the **players** when their round starts, the **provider** once per entry into `judging`, which a stuck-judging re-fire performs again and a forfeited or abandoned round never performs at all. So a re-fire *is* counted; what stays uncounted is only this section's 3 attempts **inside** one pass, deliberately, because counting them would give this frozen interface a database dependency it must not have. Full rule: `docs/GAME.md` §4.3; why: `docs/DECISIONS.md` 2026-09-19 and 2026-09-20.
...
The button on `/draw` that asks the AI what the canvas depicts (`docs/API.md` §13) runs on a third interface in `internal/judge`, and the second one the collaborator neither implements nor is asked to:
...
| `http` | **unconfigured.** The collaborator's service implements the two-image §2 contract and has no endpoint that names one drawing — he was never asked to build one. `internal/guess` gets a `nil` `Guesser` and every `/api/guess` call refuses with a `500` that names the cause, deliberately **not** falling back to `FakeGuesser`: a fabricated guess is a *more* convincing lie than a fabricated score, because a wrong guess is funny, so it reads as the feature working rather than as the feature being off. |
