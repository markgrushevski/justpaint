...
- **`GEMINI_MODEL`/`GEMINI_BASE_URL` are configurable because we cannot pin what Google moves.** The free tier's model names and quotas change on Google's own schedule; hardcoding either would turn a routine upstream rename into a code change instead of a config edit. The shipped defaults (`gemini-2.5-flash`, the public `v1beta` root) are a verified-today starting point, not a promise.
