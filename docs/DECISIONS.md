...
- **Two entry points, `ensure` and `recover`. Rejected: a single 401 interceptor in the fetch client.** An interceptor is the tidier-looking design and the wrong one: not every 401 is user-initiated. A background poll tick or a WS reconnect would throw a modal in the player's face unprompted. The caller knows whether a human is standing there waiting; the transport does not. `recover()` is the 401 form and clears the stale session FIRST — otherwise `isLoggedIn` is still true and `ensure()` resolves `true` against a session that no longer exists.
- **The gate awaits the session restore.** Three views call `fetchMe()` on mount *without* awaiting it, so a click landing inside that window read `isLoggedIn === false` for a visitor who was in fact signed in. That cost a stray 401 before; with a modal wired to it, it would accuse a signed-in user of being anonymous. `session.ready()` restores at most once and shares one request between concurrent callers.
